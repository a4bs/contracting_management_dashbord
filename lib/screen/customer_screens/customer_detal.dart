...
class CustomerDetal extends StatelessWidget {
  final CustomerModel? customer;
  CustomerDetal({super.key, this.customer});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'تفاصيل العميل'),
      body: ListView(
        children: [
          if (customer != null)
            LayoutTabletPhone(children: [build_card_customer(customer!)]),
          build_card_bills(),
        ],
      ),
    );
                  ...
                  if (customer.files != null) ...[
                    ...
                    CustomButton(
                      ...
                      onPressed: () async {
                        Get.dialog(
                          Scaffold(
                            appBar: AppBar(title: const Text('عرض الملفات')),
                            body: DisplayGroubFile(files: customer.files!),
                          ),
                        );
                      },
                    ),
  ...
  build_card_bills() {
    return FutureBuilder(
      future: billController.filterBills(BillFilter(customer: customer?.id)),
      builder: (context, snapshotBill) {
        if (snapshotBill.connectionState == ConnectionState.done &&
          ...
          BillModel bill = snapshotBill.data!.first;
          return Column(
            children: [
              LayoutTabletPhone(
                children: [build_card_unit(bill), build_card_bill(bill)],
              ),
              const SizedBox(height: 16),
              const Divider(),
              FutureBuilder(
                future: bondController.filterBonds(
                  FilterBond(customerId: customer?.id),
                ),
                builder: (context, snapshotBond) {
                  if (snapshotBond.connectionState == ConnectionState.done) {
                    return Column(
                      children: [
                        MoneyCustomerSummary(
                          transactions: snapshotBond.data ?? [],
                          itemBill: bill,
                        ),
                        const Divider(),
                        ListView.builder(
                          ...
                          itemCount: snapshotBond.data?.length ?? 0,
                          itemBuilder: (context, index) {
                            return build_card_bound_box_item(
                              snapshotBond.data![index],
                              bill,
                            );
                          },
                        ),
  ...
  build_card_unit(BillModel bill) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      decoration: BoxDecoration(
        color: Colors.white,
