...
class ArchivGroubTow extends GetView<ArchiveController> {
  ArchivGroubTow({super.key});
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Obx(() {
        final dataController = controller.archiveTypePaginationController;

        if (dataController.items.isEmpty && !dataController.showLoader.value) {
          dataController.fetchData(() => controller.getArchiveTypes());
        }
  ...
  Widget _buildProjectCard(ArchiveTypeModel item, int index) {
    ...
    return InkWell(
      onTap: () {
        ...
        controller.filterArchive.value.archiveTypeId = item.id;
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: const Icon(Icons.folder, color: Colors.amber, size: 40),
            ),
            const SizedBox(height: 16),
            Text(
              item.name,
