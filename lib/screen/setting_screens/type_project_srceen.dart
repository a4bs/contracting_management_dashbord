...
class TypeProjectSrceen extends GetView<ProjectController> {
  const TypeProjectSrceen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutuerPageWidget(
        controller: controller.projectTypeDataController,
        cardInfo: (data, index) {
          return Container(
        ...
        handelData: () {
          return controller.getAllProjectTypes();
        },
      ),
    );
  }

  void _showDialog({ProjectTypeModel? model}) {
    // Reset form state before showing dialog
    if (controller.formKeyToType.currentState != null) {
      controller.formKeyToType.currentState!.reset();
    }
            ...
            child: Column(
              ...
              children: [
                ...
                AppTextFiled(
                  validator: [
                    FormBuilderValidators.required(
                      errorText: 'الرجاء إدخال اسم المشروع',
                    ),
                  ],
                  name: 'name',
                  labelText: 'اسم النوع',
                ...
                CustomButton(
                  ...
                  text: 'حفظ التغييرات',
                  onPressed: () async {
                    if (controller.formKeyToType.currentState!
                        .saveAndValidate()) {
                      if (model == null) {
                        await controller.addProjectType(
                          controller.formKeyToType.currentState!.value,
                        );
                      } else {
